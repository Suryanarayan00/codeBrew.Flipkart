...
export default class Home extends Component {
  ...
  componentDidMount() {
    this.focusListner = this.props.navigation.addListener("focus", () => {
      if (this.props.route.params && this.props.route.params.add) {
        let { cardData } = this.state
        let index =cardData.findIndex((item) => item.id == this.props.route.params.add.id)
        if (index!=-1){
          cardData[index].num=cardData[index].num+1
          this.setState({cardData: cardData})
        }
  ...
  componentWillUnmount() {
    ...
    // this.props.navigation.navigate('main', {data: cardData})
  }
    ...
    return (
      <SafeAreaView>
        <View style={style.header}>
          {/* <TouchableOpacity onPress={this.props.navigation.navigate('login')}><Text>btn</Text></TouchableOpacity> */}
          <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around', }}>
            <Ionicons name='menu' size={30} color='white' style={{ paddingRight: 10, paddingTop: 10 }} />
            <Image style={{ width: 200, height: 50 }} source={require('../../../../assets/image/flipkart.jpg.png')} />
          </View>
          <View style={{ display: 'flex', flexDirection: 'row' }}>
            <TouchableOpacity><FontAwesome name='bell' size={17} color='white' style={{ paddingRight: 8, paddingTop: 5 }} /></TouchableOpacity>
